...
try:
    ...
    try:
        from tools.chart_generator import (
            CDGDashboardGenerator, QueryIntegratedChartGenerator, 
            create_chart_from_query_data, create_quick_chart, 
            pivot_chart_with_query_integration, handle_chart_change_request,
            validate_chart_generator, ChartFactory,
            # 🔐 NUEVAS FUNCIONES V4.4:
            get_chart_options_by_role, validate_chart_config_for_user,
            create_chart_with_confidentiality, interpret_chart_change_with_context,
            handle_chart_pivot_request, get_chart_metadata_for_frontend
        )
        CHART_GENERATOR_AVAILABLE = True
        # logger.info will be available after logger is defined below
    except Exception as e:
        CHART_GENERATOR_AVAILABLE = False
        # logger.error will be available after logger is defined below
...
# Now that logger is defined, log chart generator status if desired
if CHART_GENERATOR_AVAILABLE:
    logger.info("✅ Chart generator V4.4 cargado correctamente")
else:
    logger.error("❌ Error cargando chart generator o no disponible")
...
try:
    ...
    print(f"   - Modo: {getattr(chat_agent, 'mode', 'PRODUCTION' if IMPORTS_SUCCESSFUL else 'FALLBACK')}")
...
# AÑADIR después de otros modelos:
class ChartPivotRequest(BaseModel):
    userid: str
    message: str
    current_chart_config: Optional[Dict[str, Any]] = None
    chart_interaction_type: str = "pivot"
    user_role: Optional[str] = "GESTOR"
    gestor_id: Optional[str] = None
...
async def integration_classify_and_route(body: Dict[str, Any] = Body(...)):
    ...
    try:
        # 🔧 CORRECCIÓN: Extraer datos del request correctamente
        message = body.get("message", "")
        user_role = body.get("user_role", "GESTOR")
        ...
        # 🎯 CORRECCIÓN CRÍTICA: Pasar user_role al contexto
        context = {
            'user_role': user_role,  # ← ESTA ERA LA LÍNEA FALTANTE
            ...
            'scope': user_role.lower() if user_role else 'gestor'
        }
        
        if hasattr(chat_agent, 'classifier'):
            # Usar el clasificador con el contexto corregido
        ...
        return ok({
            ...
            'confidence': 0.1
        }, meta={'note': 'classification-error'})
...
async def chat_websocket(websocket: WebSocket, user_id: str):
    """
    🎯 WebSocket optimizado para Chat Agent v11.0 CON SISTEMA DE CONFIDENCIALIDAD + CHART GENERATOR V4.4
    """
    ...
    # 🔐 V4.4: Variables de contexto de confidencialidad para gráficos
    current_gestor_id = None
    current_centro_id = None
    confidentiality_active = True
    current_chart_config = {}  # 📊 NUEVO: Mantener configuración actual del gráfico

    # 🔐 V4.4: Función helper para enriquecer mensajes con contexto de gráficos
    def enrich_message_with_chart_context(message: str, context: dict) -> dict:
        """Enriquece mensajes con contexto de confidencialidad para gráficos"""
        try:
            return {
                'message': message,
                'user_role': user_role.value,
                'user_context': {
                    ...
                    'centro_id': current_centro_id
                },
                'confidentiality_active': confidentiality_active,
                'current_chart_config': current_chart_config,
                'websocket_version': 'V4.4_with_charts'
            }
        except Exception:
    ...
    await websocket.send_json({
        ...
        "chart_generator": "v4.4",  # 📊 NUEVO: Versión de gráficos
        "integration": "perfect_with_confidentiality_and_charts",  # ⬅️ NUEVO
        ...
        "security_features": [  # 🔐 NUEVO: Lista de funcionalidades
            ...
            "dynamic_chart_pivot",  # 📊 NUEVO
            "chart_confidentiality_controls"  # 📊 NUEVO
        ],
        "chart_features": [  # 📊 NUEVO: Capacidades de gráficos
            "conversational_chart_changes",
            "role_based_chart_options",
            "confidential_data_filtering",
            "azure_openai_interpretation"
        ],
        "ts": now_iso()
    })
    ...
    try:
        while True:
            ...
            # 📊 NUEVO V4.4: Manejo específico de mensajes de gráficos
            message_type = payload.get("type", "chat")
            
            if message_type == "chart_change":
                await handle_chart_change_websocket(websocket, payload, user_role, current_gestor_id, current_centro_id)
                continue
            elif message_type == "chart_request":
                await handle_chart_request_websocket(websocket, payload, user_role, current_gestor_id, current_centro_id)
                continue
            elif message_type == "update_chart_config":
                # 📊 Actualizar configuración actual del gráfico
                current_chart_config.update(payload.get("chart_config", {}))
                await websocket.send_json({
                    "type": "chart_config_updated",
                    "config": current_chart_config,
                    "ts": now_iso()
                })
                continue
            ...
            if "gestor-" in user_id:
                try:
                    ...
                    current_gestor_id = gestor_id  # 📊 Actualizar contexto global
            ...
            # 📊 V4.4: Añadir contexto de gráficos
            enhanced_context['chart_context'] = {
                'current_config': current_chart_config,
                'confidentiality_level': user_role.value,
                'chart_permissions': get_chart_options_by_role(user_role.value) if CHART_GENERATOR_AVAILABLE else {}
            }
            
            if gestor_id:
                enhanced_context['gestor_id'] = gestor_id
                current_centro_id = base_context.get('centro_id')  # 📊 Actualizar contexto global
            ...
            try:
                chat_message_data = {
                    "user_id": user_id,
                    "message": msg,
                    "context": enhanced_context,  # ✅ CONTEXTO ENRIQUECIDO CON ROLES Y GRÁFICOS
                ...
                msg_obj = ChatMessage(
                    ...
                    include_recommendations=req.include_recommendations,
                    context=enhanced_context,  # ✅ CONTEXTO ENRIQUECIDO CON ROLES Y GRÁFICOS
                    current_chart_config=current_chart_config  # 📊 NUEVO: Config actual
                )
                ...
                # 📊 V4.4: Si la respuesta incluye gráficos, actualizar configuración
                if isinstance(data, dict) and data.get('charts'):
                    for chart in data.get('charts', []):
                        if chart.get('config'):
                            current_chart_config.update(chart['config'])
                ...
                security_metadata = {
                    ...
                    "chart_permissions_applied": CHART_GENERATOR_AVAILABLE  # 📊 NUEVO
                }
                ...
                await websocket.send_json({
                    ...
                    "data": data, 
                    "chat_agent": "v11.0",  # ⬅️ ACTUALIZADO
                    "chart_generator": "v4.4",  # 📊 NUEVO
                    "integration": "perfect_with_confidentiality_and_charts",  # ⬅️ ACTUALIZADO
                    "gestor_context": gestor_id,  # ✅ INFO ADICIONAL
                    "security": security_metadata,  # 🔐 NUEVO: Metadata de seguridad
                    "chart_context": {  # 📊 NUEVO: Contexto de gráficos
                        "current_config": current_chart_config,
                        "available_options": enhanced_context.get('chart_context', {}).get('chart_permissions', {})
                    },
                    "ts": now_iso()
                })
                ...
                await websocket.send_json({
                    ...
                    "error_context": {
                        ...
                        "chart_generator_available": CHART_GENERATOR_AVAILABLE  # 📊 NUEVO
                    },
                    "ts": now_iso()
                })
...
# 📊 NUEVAS FUNCIONES AUXILIARES PARA EL WEBSOCKET

async def handle_chart_change_websocket(websocket: WebSocket, payload: dict, user_role, gestor_id: str, centro_id: str):
    """
    📊 V4.4: Maneja cambios de gráficos en tiempo real vía WebSocket
    """
    try:
        if not CHART_GENERATOR_AVAILABLE:
            await websocket.send_json({
                "type": "chart_error",
                "message": "Chart generator no disponible",
                "ts": now_iso()
            })
            return
        
        user_message = payload.get("message", "")
        current_config = payload.get("current_chart_config", {})
        
        # 🔐 Usar función con contexto de confidencialidad
        result = interpret_chart_change_with_context(
        ...
        # Añadir contexto WebSocket
        result['websocket_context'] = {
            'user_role': user_role.value,
            'gestor_id': gestor_id,
            'centro_id': centro_id,
            'handler': 'websocket_chart_change'
        }
        
        await websocket.send_json({
            "type": "chart_change_result",
            "data": result,
            "chart_generator": "v4.4",
            "ts": now_iso()
        })
        
    except Exception as e:
        logger.error(f"Error en chart change WebSocket V4.4: {e}")
        await websocket.send_json({
            "type": "chart_error",
...
async def handle_chart_request_websocket(websocket: WebSocket, payload: dict, user_role, gestor_id: str, centro_id: str):
    """
    📊 V4.4: Maneja solicitudes de creación de gráficos vía WebSocket
    """
    try:
        if not CHART_GENERATOR_AVAILABLE:
            await websocket.send_json({
        ...
        # 🔐 Crear gráfico con contexto de confidencialidad
        chart = create_chart_with_confidentiality(
        ...
        logger.error(f"Error en chart request WebSocket V4.4: {e}")
        await websocket.send_json({
            "type": "chart_error",
            "message": f"Error creando gráfico: {str(e)}",
            "ts": now_iso()
        })
...
# REEMPLAZAR TODO EL ENDPOINT POR ESTO:
@app.get("/charts/meta", tags=["Charts"], response_model=ApiResponse)
def charts_meta(user_role: str = Query("GESTOR", description="Rol del usuario: GESTOR o CONTROL_GESTION")):
    """
    🔐 METACONFIGURACIÓN V4.4 - Con opciones específicas por rol de usuario
    """
    try:
        if not CHART_GENERATOR_AVAILABLE:
            return ok({
                "chart_types": ["bar", "line", "pie"],
                "dimensions": {"gestor": "Gestor", "centro": "Centro"},
                "metrics": {"CONTRATOS": "Contratos", "ROE": "ROE"},
                "user_role": user_role,
                "confidentiality_enabled": False
            })
        
        # 🚀 Usar función especializada para frontend
        meta = get_chart_metadata_for_frontend()
        
        # 🔐 Añadir opciones específicas por rol
        role_options = get_chart_options_by_role(user_role)
        
        # Combinar metadatos generales con opciones específicas del rol
        meta.update({
            'user_role': user_role,
            'available_for_role': role_options,
            'confidentiality_rules': {
                'GESTOR': 'Acceso limitado a datos propios y comparativas anónimas',
                'CONTROL_GESTION': 'Acceso completo a todos los datos y métricas'
            }
        })
        ...
        logger.error(f"Error en charts/meta V4.4: {e}")
        return ok({"error": str(e)}, meta={"note": "charts_meta_error"})

# AÑADIR ESTOS NUEVOS ENDPOINTS:

@app.get("/charts/options/{user_role}", tags=["Charts"], response_model=ApiResponse)
def charts_options_by_role(user_role: str):
    """
    🔐 NUEVO V4.4: Opciones de gráficos disponibles por rol específico
    """
    try:
        if not CHART_GENERATOR_AVAILABLE:
            return ok({"options": [], "user_role": user_role, "available": False})
        
        options = get_chart_options_by_role(user_role)
        return ok(options, meta={"endpoint": "charts_options_by_role", "version": "V4.4"})
        
    except Exception as e:
        return ok({"error": str(e), "user_role": user_role})

@app.post("/charts/validate-config", tags=["Charts"], response_model=ApiResponse)
def charts_validate_config(body: dict = Body(...)):
    """
    🔐 NUEVO V4.4: Validar configuración de gráfico según permisos del usuario
    """
    try:
        if not CHART_GENERATOR_AVAILABLE:
            return ok({"valid": True, "note": "Validation skipped - chart generator not available"})
        
        config = body.get('config', {})
        user_role = body.get('user_role', 'GESTOR')
        user_context = body.get('user_context', {})
        
        validation = validate_chart_config_for_user(config, user_role, user_context)
        return ok(validation, meta={"endpoint": "charts_validate_config", "version": "V4.4"})
        
    except Exception as e:
        return ok({"error": str(e), "valid": False})

@app.get("/admin/validate-chart-generator", tags=["Admin"], response_model=ApiResponse)
def validate_chart_generator_endpoint():
    """🔧 Endpoint para validar Chart Generator V4.4"""
    try:
        if not CHART_GENERATOR_AVAILABLE:
            return ok({
                "status": "ERROR",
        ...
        return ok(validation_result, meta={"endpoint": "validate_chart_generator", "version": "V4.4"})
        
    except Exception as e:
        return ok({"status": "ERROR", "message": str(e), "version": "V4.4"})


@app.post("/charts/create-secure", tags=["Charts"], response_model=ApiResponse)
def charts_create_secure(body: dict = Body(...)):
    """
    🔐 NUEVO V4.4: Crear gráfico con contexto de confidencialidad completo
    """
    try:
        if not CHART_GENERATOR_AVAILABLE:
            return ok({"chart": {"id": "mock", "type": "error", "message": "Chart generator not available"}})
        
        query_data = body.get('data', [])
        config = body.get('config', {})
        user_role = body.get('user_role', 'GESTOR')
        ...
        return ok({"chart": chart}, meta={"endpoint": "charts_create_secure", "version": "V4.4"})
        
    except Exception as e:
        return ok({"error": str(e), "chart": None})
...
def charts_pivot(body: ChartPivotRequest):
    """
    🚀 PIVOT DE GRÁFICOS V4.4 - Con sistema de confidencialidad integrado
    """
    try:
        if not CHART_GENERATOR_AVAILABLE:
            return ok({"status": "mock", "message": "Chart generator no disponible"})
        
        # 🔐 Extraer contexto de usuario para confidencialidad
        user_role = getattr(body, 'user_role', 'GESTOR')  # Default GESTOR por seguridad
        gestor_id = getattr(body, 'gestor_id', None)
        centro_id = getattr(body, 'centro_id', None)
        
        logger.info(f"🔄 Charts pivot V4.4 para rol {user_role}")
        
        # 🚀 Usar función especializada con confidencialidad
        result = handle_chart_pivot_request(
            user_message=body.message,
            chart_config=body.current_chart_config or {},
            user_role=user_role,
            user_context={
                'gestor_id': gestor_id,
                'centro_id': centro_id,
        ...
        # Enriquecer respuesta con información del sistema
        if result.get('status') == 'success':
            result['pivot_version'] = 'V4.4'
            result['confidentiality_applied'] = True
            result['user_role'] = user_role
            
        return ok(result, meta={"pivot_type": body.chart_interaction_type, "source": "chart_pivot_v4.4"})
        
    except Exception as e:
        logger.error(f"Error en charts/pivot V4.4: {e}")
        return ok({"status": "error", "message": str(e)}, meta={"note": "chart_pivot_error"})
