...
# Configuración de paths más robusta
...
class TestComparativeQueries:
    ...
    def test_compare_precio_real_vs_std_enhanced_structure(self, comparative_queries, sample_producto_id, sample_segmento_id, sample_periodo):
        ...
        if result.data and len(result.data) > 0:
            ...
            expected_enhanced_fields = [
                'desviacion_pct', 'desviacion_absoluta', 'nivel_alerta', 'accion_recomendada',
                'tipo_desviacion', 'analisis_kpi'
            ]
            for field in expected_enhanced_fields:
                assert field in enhanced_row, f"Campo enhanced {field} faltante"
            
            assert enhanced_row['nivel_alerta'] in ['CRITICA', 'ALTA', 'MEDIA', 'NORMAL']
            assert enhanced_row['tipo_desviacion'] in ['POSITIVA', 'NEGATIVA']
    ...
    def test_ranking_gestores_por_margen_enhanced_kpi_integration(self, comparative_queries, sample_periodo):
        ...
        if result.data and len(result.data) > 0:
            ...
            kpi_fields = [
                'ranking', 'margen_neto', 'beneficio_neto', 'clasificacion_margen', 'ratio_eficiencia',
                'clasificacion_eficiencia', 'categoria_performance', 'analisis_completo'
            ]
            for field in kpi_fields:
                assert field in enhanced_data, f"Campo KPI {field} faltante"
            
            assert enhanced_data['clasificacion_margen'] in ['EXCELENTE', 'BUENO', 'ACEPTABLE', 'BAJO', 'PERDIDAS', 'SIN_INGRESOS']
            assert enhanced_data['clasificacion_eficiencia'] in ['MUY_EFICIENTE', 'EFICIENTE', 'EQUILIBRADO', 'INEFICIENTE', 'GASTOS_NULOS']
            assert enhanced_data['categoria_performance'] in ['HIGH_PERFORMER', 'GOOD_PERFORMER', 'AVERAGE_PERFORMER', 'NEEDS_IMPROVEMENT']
    ...
    def test_compare_roe_gestores_enhanced_banking_context(self, comparative_queries, sample_periodo):
        ...
        if result.data and len(result.data) > 0:
            ...
            roe_fields = ['ranking', 'roe', 'clasificacion_roe', 'benchmark_vs_sector', 'media_roe', 'desviacion_vs_media', 'analisis_roe_completo']
            for field in roe_fields:
                assert field in roe_data, f"Campo ROE enhanced {field} faltante"
            
            assert roe_data['clasificacion_roe'] in ['SOBRESALIENTE', 'BUENO', 'PROMEDIO', 'BAJO', 'NEGATIVO']
    ...
    def test_compare_eficiencia_centro_enhanced_kpi_analysis(self, comparative_queries, sample_periodo):
        ...
        if result.data and len(result.data) > 0:
            enhanced_row = result.data[0]
            centro_fields = ['ranking', 'beneficio_neto', 'margen_neto_pct', 'clasificacion_margen', 'ratio_eficiencia', 'clasificacion_eficiencia', 'interpretacion_eficiencia', 'gasto_por_contrato', 'ingreso_por_gestor', 'analisis_completo']
            for field in centro_fields:
                assert field in enhanced_row, f"Campo KPI centro {field} faltante"
            
            assert enhanced_row['clasificacion_margen'] in ['EXCELENTE', 'BUENO', 'ACEPTABLE', 'BAJO', 'PERDIDAS', 'SIN_INGRESOS']
            assert enhanced_row['clasificacion_eficiencia'] in ['MUY_EFICIENTE', 'EFICIENTE', 'EQUILIBRADO', 'INEFICIENTE', 'GASTOS_NULOS']
    ...
    @patch('src.queries.comparative_queries.iniciar_agente_llm')
    ...
    # TESTS DE FUNCIONES DE CONVENIENCIA ENHANCED
    ...
    # TESTS DE CASOS EDGE COMPARATIVOS
    ...
    def test_nonexistent_producto_segmento_handling(self, comparative_queries):
        ...
        if result is None:
            assert True  # Es válido que devuelva None para producto inexistente
        else:
            assert isinstance(result, QueryResult)
    ...
    def test_enhanced_comparative_queries_with_nonexistent_data(self, comparative_queries):
        ...
        if result is None:
            assert True  # Es válido que devuelva None para período inexistente
        else:
            assert isinstance(result, QueryResult)
    ...
    def test_invalid_periodo_format_comparative(self, comparative_queries, sample_producto_id, sample_segmento_id):
        ...
        if result is None:
            assert True  # Es válido que devuelva None para período inválido
        else:
            assert isinstance(result, QueryResult)
    ...
    def test_empty_comparison_periods(self, comparative_queries, sample_centro_id):
        ...
        if result is None:
            assert True  # Es válido que devuelva None para período sin datos
        else:
            assert isinstance(result, QueryResult)
            assert result.row_count >= 0
...
class TestComparativeQueriesKPIIntegration:
    ...
    @pytest.fixture
    def sample_producto_id(self):
        return "600100300300"  # CORREGIDO: formato string correcto
    ...
    def test_price_deviation_kpi_analysis(self, comparative_queries, sample_producto_id, sample_segmento_id, sample_periodo):
        ...
        if result.data and len(result.data) > 0:
            ...
            expected_kpi_fields = ['desviacion_pct', 'desviacion_absoluta', 'nivel_alerta']
